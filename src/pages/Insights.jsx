...
export default function Insights() {
  ...
  return <div className="space-y-6"><header><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-primary">Your operational memory</p><h1 className="font-heading text-5xl font-bold">Shift intelligence</h1><p className="mt-2 text-muted-foreground">Every metric below is calculated from your local SQLite records.</p></header>
    {ordersLoading || eventsLoading || placesLoading ? <Loading /> : error ? <div className="rounded-2xl bg-destructive/10 p-5 text-destructive">{error.message}<button onClick={() => { reloadOrders(); reloadEvents(); reloadPlaces(); }} className="ml-3 underline">Retry</button></div> : <>
      ...
      <section className="overflow-hidden rounded-3xl border border-border bg-card"><div className="flex flex-wrap items-end justify-between gap-3 p-6"><div><h2 className="font-heading text-3xl font-bold">GPS activity map</h2><p className="text-sm text-muted-foreground">OpenStreetMap tiles with your locally captured route and milestone points</p></div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-secondary-foreground">{gps.length} points</span></div>{gps.length === 0 || !center ? <div className="grid min-h-72 place-items-center border-t border-border p-8 text-center"><div><ApperIcon name="Map" className="mx-auto mb-3 text-muted-foreground" /><p className="text-sm text-muted-foreground">No GPS points yet. Open Live Run and keep location access enabled during a delivery.</p></div></div> : <div className="h-[480px] border-t border-border"><MapContainer center={center} zoom={12} scrollWheelZoom className="h-full w-full"><TileLayer attribution="&copy; OpenStreetMap contributors" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />{gps.map(point => <CircleMarker key={point.id} center={[Number(point.latitude), Number(point.longitude)]} radius={Math.min(18, 7 + Number(point.accuracy_m || 0) / 25)} pathOptions={{ className: 'gps-point' }}><Tooltip>{point.event_type} · ±{point.accuracy_m ? `${Math.round(point.accuracy_m)} m` : 'unknown accuracy'}</Tooltip></CircleMarker>)}</MapContainer></div>}</section>
    </>}
  </div>;
}
